"""
Example: How to call the gocar-be-scraper actor using Apify API Client

For full documentation of the Apify Client for Python, see: https://docs.apify.com/api/client/python
For full documentation of the actor, see: https://apify.com/lexis-solutions/gocar-be-scraper
...
def run_actor():
    """Run the gocar-be-scraper actor and retrieve results."""
    ...
    actor_id = "lexis-solutions/gocar-be-scraper"
    print(f"Starting actor: {actor_id}")

    run = client.actor(actor_id).call(
        run_input={
            "startUrls": [{"url":"https://gocar.be/en/search?gbrands=Mercedes~Rover~BMW&gmodels=1%20Series&fuel=Electric~Petrol"}],
            "maxItems": 1,
